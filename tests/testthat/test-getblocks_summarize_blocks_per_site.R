test_that("getblocks_summarize_blocks_per_site does not crash", {
  expect_no_error({
    junk <- capture_output({
      ...
      y = getblocks_summarize_blocks_per_site(testoutput_getblocksnearby_1000pts_1miles, "blockid" )
...
test_that("getblocks_diagnostics does not crash", {
  expect_no_error({
    junk <- capture.output({ 
      ...
      x = getblocks_diagnostics(testoutput_getblocksnearby_1000pts_1miles, detailed = TRUE, see_pctiles = TRUE)
  ...
  expect_true(
    class(x) == 'list' & "sitecount_unique_out" %in% names(x)
  )
})
