    ...
    shiny::tabsetPanel(
      ...
      shiny::tabPanel(title =  " ", # semi-hidden tab - if you click on the empty space you can open this tab
        ...
        numericInput(ns('default_miles'), label = "Default miles radius",  # this will get ignored here probably, and just set by reactive passed to module
                     min = 0.25,
                     value = default_default_miles, # value =    1, # default_default_miles was set in module-specific global_defaults_*.R
...
mod_ejscreenapi_server <- function(id, session,
  ...
  # stopifnot(!is.reactive(parameter2))
      ...
      source(system.file("global_defaults_ejscreenapi.R", package = "EJAM"))  ### IMPORTANTLY THESE MUST STAY WITHIN THE MODULE NAMESPACE, AND NOT BE AVAILABLE TO OR INTERFERE WITH MAIN APP'S DEFAULTS/ETC.
...
# Compare and consolidate/ merge these - unclear which is newer, and they may each have some updates others dont:
    ...
    on.exit({ # these are leftovers created by the shiny app via global_defaults_*.R probably
    ...
    pts <- shiny::reactive({
      ...
      if (is.null(input$pointsfile)) {
        pts_filecontents <- default_points_shown_at_startup_react()  # defined in global_defaults_*.R
        # }
      } else {
        ...
        if (confirmed_read_and_clean_points_works) {
          ...
          if (0 == length(pts_filecontents)) {
            pts_filecontents <- default_points_shown_at_startup  # defined in global_defaults_*.R # NOT SURE THIS IS POSSIBLE
          } else {
    ...
    output$speed <- renderText({speedmessage(NROW(pts()), perhourslow = perhourslow, perhourfast = perhourfast, perhourguess = perhourguess)})  # speedmessage defined as function in R folder, default variables defined in global_defaults_*.R
    ################################################################################################### #
    ...
    output$plot1out <- shiny::renderPlot({
      ...
      ## plot_boxplot_ratios(us.ratios$ratios_d)
      outplot <- plot_boxplot_ratios(us.ratios$ratios_d, 'pctlowinc', '% low income', wheretext = paste0("Within ", out$radius.miles[1]," miles of"))
      outplot
      # plot(us.ratios)
    } )
