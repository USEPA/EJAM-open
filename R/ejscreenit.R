...
ejscreenit <- function(x, y=NULL, radius = 3, maxradiusmiles=10,
  ...
  if (calculate_ratios) {
    ...
    outplot <- plot_boxplot_ratios(us.ratios$ratios_d, 'ratio.to.avg.pctlowinc', '% low income', wheretext = out$radius.miles[1])
    # outplot <- plot_boxplot_ratios(st.ratios$ratios_d, 'ratio.to.state.avg.pctlowinc', '% low income', wheretext = out$radius.miles[1])
    ...
    if (save_plot) {
      if (interactive()) {
        ...
        # plot_boxplot_ratios(us.ratios$ratios_d, 'pctlowinc', '% low income')
        print(outplot)
        # outplot
        dev.off()
      } else {
