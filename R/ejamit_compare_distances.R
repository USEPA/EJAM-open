...
# # internal:
#
# ejamit_compare_distances2plot - used by the exported ejam2barplot_distances
...
#'
#' @details used by [ejam2barplot_distances()] to plot results of [ejamit_compare_distances()]
#'
...
ejamit_compare_distances2plot <- function(results_bydistance,
  ...
  #  it could be renamed (and exported as)  plot_barplot_distances()  ?  like  plot_barplot_sites() and ejam2barplot_distances() already named
  #  or could be renamed (and exported as)  plot_indicators_bydistance() ? like plot_demogshare_by_distance()
  ...
  mycolors <- palette.colors(NCOL(x), recycle = TRUE) # without recycle=T, only allows 9 colors, limit of default palette
  for (i in 1:(NCOL(x))) {
    if (i == 1) {
      plot(x = radii, y = x[, i], type = "b", col = mycolors[i], pch = i,
           sub = subtext,
           # x labels are radii
      ...
      points(x = radii, y = x[, i], type = "b", col = mycolors[i], pch = i)
    }
  }
  legend("topright", legend = paste0("", fixcolnames(names(x), "r", "shortlabel"),""),
         pch = 1:NCOL(x), col = palette.colors(NCOL(x), recycle = TRUE)
         # fill = palette.colors(NCOL(x), recycle = TRUE)
         )
...
#' @param ylim  optional, see [ejamit_compare_distances()]
#' @param n  optional, see [ejamit_compare_distances()]
#' @param ... optional, other parameters passed to plot
#' @return text vector length n, naming which indicators most strongly
#'   increase as you get closer to the site(s)
#' @examples
