...
get_facility_info_via_ECHO <- function(p_ncs=NULL, qcolumns=c(16,17), output='JSON', otherparameters=NULL,
  ...
  if (getcsv) {
    ...
    mycsv <- try(httr::GET(mycsv), silent = TRUE)
    if (inherits(mycsv, "try-error") || mycsv$Message != "Success") {stop('csv query failed')}
  ...
  requested <- try(httr::GET(urlquery), silent = TRUE)  # THIS FAILS NOW? ********
