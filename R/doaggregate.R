...
doaggregate <- function(sites2blocks, sites2states_or_latlon=NA,
                        radius=NULL,
                        countcols=NULL, wtdmeancols=NULL, calculatedcols=NULL, calctype_maxbg=NULL, calctype_minbg=NULL,
                        subgroups_type='nh',
  ...
  if (is.null(countcols)) {
    ...
    countcols <- unique(intersect(
      map_headernames$rname[calctype(map_headernames$rname) %in%  "sum of counts"], 
      names(blockgroupstats)
    ))
    ## very few of these are not in blockgroupstats, actually:  and it is ok to exclude these from calculation of sums. area_sqmi is handled separately
    # setdiff(map_headernames$rname[calctype(map_headernames$rname) %in%  "sum of counts"], names(blockgroupstats))
    # [1] "nonmins"        "female1849"     "inputAreaMiles" "area_sqmi"     
  ...
  if (is.null(calctype_maxbg)) {
    calctype_maxbg <- map_headernames$rname[calctype(map_headernames$rname) %in%  c("flag", "maxbg")]
    ##   "flag" like yesno_airnonatt = 1 means some part of blockgroup is, so max makes sense if you assume intersected portion can be 
    ## approximated as called yes as long as any of bg even outside intersection part, was yes.
  }
  if (is.null(calctype_minbg)) {
    calctype_minbg <- map_headernames$rname[calctype(map_headernames$rname) %in%  c( "minbg")]
    # maybe none? only works for those already in blockgroupstats, not derived scores like distance to nearest site.
  }
  ...
  # weighted sums are used here to assume/approximate the total count is evenly distributed across the blocks in a blockgroup
  ...
  ##################################################### #
  # * MIN or MAX of blockgroupstats indicators ####
  ##################################################### #
  
  ##################### # 
  # calctype_maxbg  (e.g., the 1/0 code for blockgroups flagged as in area of certain type like nonattain or tribal
  colnames_of_this_calc_type <- calctype_maxbg
  this_function <- collapse::fmax

  # copy of somewhat generic aggregation code:
  #
  if (length(colnames_of_this_calc_type) > 0) {
  results_bysite_this_calc_type <- sites2bgs_plusblockgroupdata_bysite[, lapply(.SD, function(x) {
    this_function(x)
  }), .SDcols = colnames_of_this_calc_type,
  by = .(ejam_uniq_id)]
  results_overall_this_calc_type <- sites2bgs_plusblockgroupdata_overall[, lapply(.SD, function(x) {
    this_function(x)
  }), .SDcols = colnames_of_this_calc_type]
  results_bysite  <- merge(results_bysite, results_bysite_this_calc_type, by = "ejam_uniq_id")
  results_overall <- cbind(results_overall, results_overall_this_calc_type)
  }
  ##################### # 
  # calctype_minbg
  colnames_of_this_calc_type <- calctype_minbg
  this_function <- collapse::fmin
