...
ejscreenapi_plus <- function(x, y=NULL, radius = 3, unit ='miles', wkid=4326,
  ...
  if (is.null(mapping_for_names)) {mapping_for_names <- map_headernames} # should be available as data via package or loaded via global_defaults_*.R
  ...
  if (interactive() & verbose) {
    ...
    # plot_boxplot_ratios(calc_ratios_to_avg(x), wheretext = "nearby")
  }
