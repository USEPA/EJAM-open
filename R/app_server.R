...
app_server <- function(input, output, session) {
  ...
  observe({
    if (length(EJAM:::global_or_param("default_shiny.testmode")) > 0  ) { # allow params in run_app() to override default
      if (!EJAM:::global_or_param("default_shiny.testmode")) {
  ...
  # output$max_mb_upload_react_txt <- renderText({
  #   paste0("Maximum file size allowed for upload: ", max_mb_upload_react(), " MB")
  # })
  ...
  data_up_shp <- reactive({
    ...
    if (golem::get_golem_options("use_shapefile_from_any")) {
      ###################################### #
      # newer way ... uses shapefile_from_any() which uses shapefix()
      allowed_extensions <- golem::get_golem_options("default_shp_oktypes_1") # c("zip", "gdb", "geojson", "json", "kml", "shp") # see shapefile_from_any()
  ...
  ## reactive: latlon (typed in on-screen via MODULE) *** ####
  ...
  #############################################################################  #
  ## reactive: latlon (from file upload) ####
  ...
  #############################################################################  #
  ## reactive: latlon by FRS registry IDs (from file upload) ####
  ...
  #############################################################################  #
  ## reactive: latlon by NAICS (selected from list) ####
  ...
  #############################################################################  #
  ## reactive: latlon by EPA Program IDs (selected from list) ####
  ...
  #############################################################################  #
  ## reactive: latlon by SIC (selected from list) ####
  ...
  ## reactive: places by FIPS (selected from list) via module fipspicker ####
  
  # data_fips_place() is a reactive reactiveVal() sort of like the reactive() data_up_fips().
  #  It is updated by an observer only when input$fipspicker_done_button is clicked, not in response to any uploaded file.
  #  It gets its value from a shiny module's output, not a pulldown menu in the main app.
  #  So unlike data_up_fips() that updates based on file upload button input$ss_upload_fips,
  ...
  observe({ # only when Done button clicked
  ...
  ## reactive: latlon by MACT subpart (selected from list) ####
  ...
  data_uploaded <- reactive({
    ...
    } else if (current_upload_method() == 'FIPS'           ) {data_up_fips()    # blocks in census units, not lat/lon values
    } else if (current_upload_method() == 'FIPS_PLACE'     ) {data_fips_place() # blocks in census units, not lat/lon values
    } else if (current_upload_method() == 'SHP'            ) {data_up_shp()     # blocks in polygons,     not lat/lon values
    }
  })
  #############################################################################  #
  ...
  observe({
    ...
    if (!(current_upload_method() %in% c('FIPS', 'FIPS_PLACE', 'SHP'))) {
  ...
  # intersect(as.vector(default_choices_for_type_of_site_category), as.vector(default_choices_for_type_of_site_upload))
  # x = union(as.vector(default_choices_for_type_of_site_category), as.vector(default_choices_for_type_of_site_upload))
  ...
  # __output: render map of uploaded  ####
  ...
  # ________ RESULTS OVERALL -- SUMMARY REPORT  __________ ####
  #. ####
  # HTML REPORT - FOR UI ####
  # *** The code here builds the html report shown in the shiny app but overlaps with code used to download it.
  ## *** This code should be harmonized with and/or merged with/replaced by functions like
  ## report_community_download(), which is used by server for download, and with ejam2report()
  ...
  #############################################################################  #
  # ### summary header is stored in a reactive-- OBSOLETE/UNUSED? see sanitized_standard_analysis_title()
  # summary_title <- reactiveVal(NULL)
  #
  # ### summary header, reactive summary_title(), is updated-- OBSOLETE/UNUSED?see sanitized_standard_analysis_title()
  ...
  ## *MAP ALL SITES (for summary report) ####
  
  report_map <- reactive({
    
    ## *** This code should be harmonized with and/or replaced by function like mapfast()
    # inputs/needs:
    # data_uploaded() if shp,  data_processed(),
    # submitted_radius_val(), submitted_upload_method(),
    # sanitized_bt_rad_buff(), submitted_radius_val(),
    # isTRUE(getOption("shiny.testmode")), input$circleweight_in, circle_color
    ...
    if (submitted_upload_method() == "SHP") {
      ...
      if (!(submitted_upload_method() %in% c('FIPS', 'FIPS_PLACE'))) {
        ...
        ## similar to previous map but remove controls and only add circles, not circleMarkers
            ...
            mapfast(data_processed()$results_bysite, 
            ...
            if (all(FTYPES %in% c("state", "county", "city", "tract", "blockgroup"))) {
              ...
              fips_shapes <- try( shapes_from_fips(data_uploaded()) )
              ...
              warning('Cannot map FIPS types other than state, cities, counties, Census tracts, or Census blockgroups currently.')
  ...
  ##   this was resetting map too often in response to checkbox  ***
  # observeEvent(eventExpr = {
  ...
  observe({
    ...
    if (current_upload_method() == "SHP") {
      ...
      leaflet::leafletProxy(mapId = 'an_leaf_map', session) %>% 
        ...
        shps <- shapes_from_fips(fips = data_uploaded())

        leaflet::leafletProxy(mapId = 'an_leaf_map', session) %>%
          map_shapes_leaflet_proxy(shapes = shps,popup = popup_from_df(shps %>% sf::st_drop_geometry()))
  ...
  ## ASSEMBLE via build_community_report() ####
  ...
  output$comm_report_html <- renderUI({
    ...
    if (sitetype %in% "latlon") {
      area_in_square_miles <- nsites * area_sqmi(df = data_processed()$results_overall)
    } else {
      if (sitetype %in% "shp") {
    ...
    full_page <- build_community_report(
      ...
      extratable_list_of_sections = golem::get_golem_options("default_extratable_list_of_sections"),
      ## CANNOT USE SELECTIZE TO GET A NAMED LIST LIKE THIS: 
      # extratable_list_of_sections = input$extratable_list_of_sections, # default_extratable_list_of_sections, # or could use input$ in advanced tab
  ...
  ## ASSEMBLE via report_community_download() ####
  ...
  community_download <- function(file, row_index = NULL) {
    ...
    report_community_download(file = file,
                              ...
                              input_extratable_title =         input$extratable_title, # above the table, not in the upper left cell
                              input_extratable_title_top_row = input$extratable_title_top_row, # upper left cell
  ...
  ## OVERALL REPORT downloadHandler ####
  
  report_fname <- reactiveVal()
  ...
  #############################################################################  #
  # .  ####
  # ______ RESULTS BY SITE -- DETAILED RESULTS ______ ####
  
  ## 1-SITE REPORT downloadHandler ####
  ...
  output$community_download_individual <- downloadHandler(
    ...
    content = function(file) {
      ...
      # rather than happening here, a separate observer of the 1-site buttons does something like  this:
      # isolate({
      #  sitenumber <- as.numeric(gsub('button_','', cur_button()))
      #  community_download(file, row_index = sitenumber)
      # })
  ...
  #. ## ##
  ## INTERACTIVE TABLE of sites = output$interactive_table from data_processed()  ####
  ...
  output$report_version_date <- renderUI({
    ...
    p(style = "margin-bottom: 0",
      paste("Version",
            golem::get_golem_options("ejam_app_version"),
            "| Report created on",
            ifelse(
  ...
  #############################################################################  #
  # ~--------------------------- ###
  # ______ FULL REPORT (Long Word doc) _________ ####
  # .  ## ##
  ...
  # (to get batch via API)  see default_hide_ejscreenapi_tab in global_defaults_*.R etc.
