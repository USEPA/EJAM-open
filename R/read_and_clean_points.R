...
#' WORK IN PROGRESS THAT WAS DRAFTED
...
read_and_clean_points <- function(filepath, default_points=NULL) {
  ...
  if (0 == length(pts_filecontents)) {
    pts_filecontents <- default_points  # was defined in global_defaults_*.R
  } else {
      ...
      if ('FacLong' %in% names(pts_filecontents) & 'FacLat' %in% names(pts_filecontents)) {
        ...
        if ('registry_id' %in% names(pts_filecontents)) {
          ...
          if ('pgm_sys_id' %in% names(pts_filecontents)) {
            ...
            pts_filecontents <- default_points  # defined in global_defaults_*.R   This line is so default example is shown instead of uploaded file that does not have correct columns
          }
        }
      }
    }
